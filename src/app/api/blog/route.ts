...
export async function GET(req: NextRequest) {
  ...
  if (all === "1") {
    ...
    return NextResponse.json(await db.blog.getAll());
  }

  return NextResponse.json(await db.blog.getPublished());
}
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    if (rawData.tags) data.tags = (rawData.tags as string[]).map((t) => sanitizeInput(t));
    ...
    const updated = await db.blog.update(id, data);
